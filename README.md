...
The watch relies on blocking queries, adding the x-consul-index header as per [Consul's http api documentation](https://www.consul.io/docs/agent/http.html)

If a blocking query is dropped due to a crash or disconnection from the consul instance, watch will attempt to reinitiate the blocking query with logarithmic backoff.
